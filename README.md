...
- `SECURITY.md`, `LICENSE`, `CONTRIBUTING.md`, `NOTICE.md`, `CODEOWNERS`, `THREAT_MODEL_AND_ETHICS.md` (repo root): Required by `DocumentationGate`'s documentation gate (`REQUIRED_FILES`). `LICENSE` is MIT. `NOTICE.md` carries this program's ORPHEUS attribution boilerplate. `THREAT_MODEL_AND_ETHICS.md` covers the amplification risk unique to a generator tool. Distinct from anything shipped inside `templates/python-secure/` -- generated projects do not currently receive their own copies of these.
...
- `.github/workflows/validate.yml`: `lint` (`make lint`, Ruff -- this repo's own root lint tooling;
  ...
  (GovernanceContracts), neither of which belongs in a public repo's CI (see `AlignmentHarness`'s
...
- References `../GovernanceContracts/schemas/` for mission and policy validation.
- Expects `../AlignmentHarness/`, `../RiskGate/`, `../DevilsAdvocate/` as sibling repos for CI integration.
- Contracts compatibility: `contracts-v1` (see `GovernanceContracts`' own Versioning and Compatibility
  Policy for what a MAJOR/MINOR/PATCH bump on that tag means for this repo).

## Checking a Real Change (`make check`)

`make check` runs `ComplianceRunner`'s full evaluate pipeline (Harness + PolicyEngine +
...
bundle path for the full evidence/decision log. `ComplianceRunner` (the orchestrator this target
